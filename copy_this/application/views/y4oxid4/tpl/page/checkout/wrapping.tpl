[{*
#########################################################################
#	$Id: 8ea2ca93e090e0bc16a9c9cbee020a3013d8670e $
...
[{include file="layout/header.tpl" title=$template_title location="WRAPPING_LOCATION"|oxmultilangassign}]
    ...
    <form name="basket" action="[{ $oViewConf->getSelfActionLink() }]" method="post">
        ...
        <table class="wrapping">
          ...
          [{assign var="basketproduct" value=$basketitemlist.$basketindex }]
            ...
            <tr valign="top">
              ...
              <td class="brd"></td>
              <td>
                  <a id="pic_[{$basketitem->getProductId()}]_[{$smarty.foreach.testArt.iteration}]" class="picture" href="[{$basketitem->getLink()}]">
              ...
              <td>
                <div class="art_title"><a id="title_[{$basketitem->getProductId()}]_[{$smarty.foreach.testArt.iteration}]" rel="nofollow" href="[{$basketitem->getLink()}]">[{$basketitem->getTitle()}]</a></div>
              ...
              <td colspan="2">
                <table width="100%" class="wrapping_items">
                  <tr onclick="JavaScript:document.getElementsByName('wrapping[[{$basketindex}]]')[0].checked=true;">
                    <td><input id="WrapItem_[{$basketitem->getProductId()}]_NONE" type="radio" name="wrapping[[{$basketindex}]]" value="0" [{ if !$basketitem->getWrappingId()}]CHECKED[{/if}]></td>
                    ...
                    [{foreach from=$oView->getWrappingList() item=wrapping name=Wraps}]
                      <tr onclick="JavaScript:document.getElementsByName('wrapping[[{$basketindex}]]')['[{$ictr}]'].checked=true;">
                        <td><input id="WrapItem_[{$basketitem->getProductId()}]_[{$smarty.foreach.Wraps.iteration}]" type="radio" name="wrapping[[{$basketindex}]]" value="[{$wrapping->oxwrapping__oxid->value}]" [{ if $basketitem->getWrappingId() == $wrapping->oxwrapping__oxid->value}]CHECKED[{/if}]></td>
                        <td id="WrapItemName_[{ $basketitem->getProductId()}]_[{$smarty.foreach.Wraps.iteration}]">[{$wrapping->oxwrapping__oxname->value}]</td>
                        <td id="WrapItemPrice_[{ $basketitem->getProductId()}]_[{$smarty.foreach.Wraps.iteration}]" align="right">[{$wrapping->getFPrice()}] [{ $currency->sign}]</td>
                      </tr>
                      <tr onclick="JavaScript:document.getElementsByName('wrapping[[{$basketindex}]]')['[{$ictr}]'].checked=true;">
                        <td></td>
                        <td colspan="2">
                  ...
                  <td>
                      <div class="cardbox" onclick="JavaScript:document.getElementsByName('chosencard')[0].checked=true;">
                          <div class="card_title">
                              <input id="CardItem_NONE" type="radio" class="chbox" name="chosencard" value="0" [{ if !$oxcmp_basket->getCardId() }]CHECKED[{/if}]>
                  ...
                  <td>
                    ...
                    [{/if}]
                     ...
                     <div class="cardbox" onclick="JavaScript:document.getElementsByName('chosencard')[[{counter}]].checked=true;">
                        <div class="card_title">
                            <input id="CardItem_[{$smarty.foreach.GreetCards.iteration}]" type="radio" class="chbox" name="chosencard" value="[{$card->oxwrapping__oxid->value}]" [{ if $oxcmp_basket->getCardId() == $card->oxwrapping__oxid->value}]CHECKED[{/if}]>
                            <span id="CardItemNamePrice_[{$smarty.foreach.GreetCards.iteration}]">[{$card->oxwrapping__oxname->value}] ([{$card->getFPrice() }] [{ $currency->sign}])</span>
                        </div>
     ...
     [{/if}]
      ...
      <div class="bar prevnext">
          <div class="right">
              <input id="BackToOrder" type="submit" value="[{ oxmultilang ident="WRAPPING_BACKTOORDER" }]">
          </div>
      </div>
