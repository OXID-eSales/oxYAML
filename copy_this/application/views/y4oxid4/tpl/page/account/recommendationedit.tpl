[{*
#########################################################################
#	$Id: 5644705413700566cb64d74c0813c02091229376 $
...
[{include file="layout/header.tpl" title=$template_title location="ACCOUNT_RECOMM_LOCATION"|oxmultilangassign|cat:$template_title}]

[{include file="inc/account_header.tpl" active_link=8 }]
[{assign var="actvrecommlist" value=$oView->getActiveRecommList() }]
<strong id="recomListHeader1" class="boxhead">[{if $actvrecommlist}][{$actvrecommlist->oxrecommlists__oxtitle->value}][{else}][{ oxmultilang ident="ACCOUNT_RECOMM_NEWLIST" }][{/if}]</strong>
<div class="box info">
    <form action="[{ $oViewConf->getSelfActionLink() }]" name="saverecommlist" method="post">
      ...
      <div style="right">
        <span class="btn"><input id="recomListSave" type="submit" value="[{ oxmultilang ident="ACCOUNT_RECOMM_SAVE" }]" class="btn"></span>
      </div>
...
<div class="bar prevnext">
    <form action="[{ $oViewConf->getSelfActionLink() }]" name="order" method="post">
      <div>
          ...
          <div class="right">
              <input id="BackToShop" type="submit" value="[{ oxmultilang ident="ACCOUNT_RECOMM_BACKTOSHOP" }]">
          </div>
      </div>
    </form>
</div>
