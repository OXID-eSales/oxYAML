[{*
#########################################################################
#	$Id: 02c5ac70aac2fec4f1387ab7005c2ac7d6f991ff $
...
[{include file="layout/header.tpl" title=$template_title tree_path=$oView->getTreePath()}]

[{if $oView->getActiveRecommList() }]
    [{assign var="_actvrecommlist" value=$oView->getActiveRecommList() }]

    <strong id="recommlistHeaderAuthor" class="head">
    ...
    <div class="box info">
      ...
      <div id="recommlistDesc" class="recomlistdesc">
...
[{if $oView->isReviewActive() }]
    <strong class="boxhead" id="reviewHeader">[{ oxmultilang ident="RECOMMLIST_LISTREVIEW" }]</strong>
    <div id="review" class="box info">
      [{ if $oxcmp_user }]
        <form action="[{ $oViewConf->getSelfActionLink() }]" method="post" id="rating">
            <div id="write_review">
                ...
                <span class="btn"><input id="reviewSave" type="submit" value="[{ oxmultilang ident="RECOMMLIST_SAVEREVIEW" }]" class="btn"></span>
            </div>
        </form>
      ...
      [{else}]
        <a id="Reviews_login" rel="nofollow" href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account" params="recommid="|cat:$_actvrecommlist->getId()|cat:"&amp;sourcecl="|cat:$oViewConf->getActiveClassName()|cat:$oViewConf->getNavUrlParams() }]" class="fs10"><b>[{ oxmultilang ident="RECOMMLIST_LOGGINTOWRITEREVIEW" }]</b></a>
      [{/if}]
      ...
      [{if $oView->getReviews() }]
        [{foreach from=$oView->getReviews() item=review name=ReviewsCounter}]
          <dl class="review">
            <dt>
                <span id="ReviewName_[{$smarty.foreach.ReviewsCounter.iteration}]" class="left"><b>[{ $review->oxuser__oxfname->value }]</b> [{ oxmultilang ident="RECOMMLIST_WRITES" }]</span>
                <span id="ReviewTime_[{$smarty.foreach.ReviewsCounter.iteration}]" class="right param"><b>[{ oxmultilang ident="RECOMMLIST_TIME" }]</b>&nbsp;[{ $review->oxreviews__oxcreate->value|date_format:"%H:%M" }]</span>
                <span id="ReviewDate_[{$smarty.foreach.ReviewsCounter.iteration}]" class="right param"><b>[{ oxmultilang ident="RECOMMLIST_DATE" }]</b>&nbsp;[{ $review->oxreviews__oxcreate->value|date_format:"%d.%m.%Y" }]</span>
                <span id="ReviewRating_[{$smarty.foreach.ReviewsCounter.iteration}]" class="right param">[{if $review->oxreviews__oxrating->value }]<b>[{ oxmultilang ident="RECOMMLIST_RATING" }]</b>&nbsp;[{ $review->oxreviews__oxrating->value }][{/if}]</span>
            </dt>
            <dd id="ReviewText_[{$smarty.foreach.ReviewsCounter.iteration}]">
